...
export default function App() {
  ...
  const handleSaveNote = async () => {
    ...
    try {
      ...
      setNoteMessage({ text: 'Error connecting to backend.', type: 'error' });
    } finally {
      setSavingNote(false);
  ...
  const handleSendChatMessage = async (presetText = null) => {
    ...
    try {
      ...
      setChatHistory([...updatedHistory, { sender: 'ai', message: 'Sorry, I failed to process that request. Make sure backend is running.' }]);
    } finally {
      setChatting(false);
    }
  };
  ...
  const handleCompleteReminder = async (reminderId) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/reminders/${reminderId}/complete`, { method: 'POST' });
      const data = await response.json();
      if (data.success) {
        fetchReminders();
      }
  ...
  const handleViewNoteDetails = async (note) => {
    ...
    // Fetch related notes by searching for the current note's summary/content
    try {
      const response = await fetch(`${API_BASE_URL}/api/search`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await response.json();
      // Filter out the selected note from the related results
        ...
        <View style={styles.overlayContainer}>
          <View style={styles.detailGlassCard}>
            ...
            <ScrollView style={styles.detailBody} showsVerticalScrollIndicator={false}>
              ...
              {/* Related Notes (Knowledge Proximity) */}
                ...
                {loadingRelated ? (
                  ...
                  relatedNotes.map(rn => (
                    <TouchableOpacity 
                      ...
                      onPress={() => handleViewNoteDetails(rn)}
                    >
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 4 }}>
                        <Text style={styles.relatedCategoryText}>{rn.category}</Text>
                        <Text style={styles.relatedMatchScore}>{Math.round(rn.similarity * 100)}% match</Text>
                      </View>
                      <Text style={styles.relatedContentText} numberOfLines={2}>{rn.content}</Text>
                    </TouchableOpacity>
                  ))
                ) : (
                  <Text style={styles.relatedPlaceholder}>No semantically close notes found.</Text>
                )}
              </View>
            </ScrollView>
